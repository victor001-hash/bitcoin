...
static void OrphanageEraseAll(benchmark::Bench& bench, bool block_or_disconnect)
{
    ...
    // Tranactions with 9 inputs maximize the computation / LatencyScore ratio.
    const unsigned int INPUTS_PER_TX{9};
    const unsigned int NUM_PEERS{125};
    const unsigned int NUM_TXNS_PER_PEER = node::DEFAULT_MAX_ORPHANAGE_LATENCY_SCORE / NUM_PEERS;
    ...
    // All the block inputs are spent by the orphanage transactions.
    // Jumping by 3 for each tx ensures we cover all of the inputs assigned to this peer.
    assert(3 * NUM_TXNS_PER_PEER + INPUTS_PER_TX - 1 >= INPUTS_PER_PEER);
    // Each start_input is within the peer's range, otherwise we may have duplicate transactions.
    assert(3 * NUM_TXNS_PER_PEER < INPUTS_PER_PEER);
    ...
    for (NodeId peer{0}; peer < NUM_PEERS; ++peer) {
        ...
        for (unsigned int txnum{0}; txnum < node::DEFAULT_MAX_ORPHANAGE_LATENCY_SCORE / NUM_PEERS; ++txnum) {
            ...
            const unsigned int start_input = peer * INPUTS_PER_PEER + txnum * 3;
    ...
    // They spend a total of 27,000 inputs (9605 unique ones)
