...
class TxOrphanageImpl final : public TxOrphanage {
    ...
    struct Announcement
    {
        ...
        TxOrphanage::Count GetLatencyScore() const {
            return 1 + std::floor<TxOrphanage::Count>(m_tx->vin.size() / 10);
        }
    };
    ...
    struct PeerDoSInfo {
        ...
        * - CPU score (ratio of total latency score / max allowed latency score)
...
void TxOrphanageImpl::Erase(Iter<Tag> it)
{
    ...
    if (IsUnique(m_orphans.project<ByWtxid>(it))) {
        ...
        m_unique_orphan_bytes -= it->GetMemUsage();
...
bool TxOrphanageImpl::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    if (brand_new) {
        ...
        m_unique_orphans += 1;
        m_unique_orphan_bytes += ret.first->GetMemUsage();
        m_unique_rounded_input_scores += ret.first->GetLatencyScore() - 1;
