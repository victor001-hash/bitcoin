            ...
            CallOneOf(
                ...
                [&] { // AddTx
                    ...
                    if (peer_is_protected && !have_tx_and_peer &&
                        ...
                        orphanage->LatencyScoreFromPeer(peer_id) + std::floor<node::TxOrphanage::Count>(tx->vin.size()) + 1 > honest_latency_limit)) {
...
FUZZ_TARGET(txorphanage_sim)
{
    ...
    // (which may have simple or complex topology, and may have duplicate txids with distinct wtxids, but never with
    // more than 9 inputs), and up to 16 peers. The scenario is performed both on a real TxOrphanage object and the
    // behavior is compared with a naive reimplementation (just a vector of announcements) where possible, and tested
    // for desired properties where not possible.
        ...
        if (t > 0 && rng.randrange(4) == 0) {
            ...
            for (auto& [child, parent] : deps) {
                if (child == t) {
                    ...
                    // Do not construct transactions with more than 9 inputs to avoid latency score diverging from announcement count.
                    if (tx.vin.size() >= 9) break;
                }
            }
        ...
        while (true) {
            if (sim_announcements.size() < MAX_ANN && command-- == 0) {
                ...
                for (auto& [wtxid, peer] : added) {
                    ...
                    child_wtxids.erase(child_wtxid_it);
                }
                // Verify that all child wtxids which did not occur at least once in the AddChildrenToWorkSet result
                // are wtxid which were already reconsiderable (for some peer).
                ...
                while (true) {
                    bool oversized = sim_announcements.size() > real->MaxGlobalLatencyScore();
                    if (!oversized) {
                        // Count global usage and number of peers.
                        node::TxOrphanage::Usage total_usage{0};
                        for (unsigned tx = 0; tx < NUM_TX; ++tx) {
                            if (have_tx_fn(tx)) total_usage += GetTransactionWeight(*txn[tx]);
                        }
                        auto num_peers = count_peers_fn();
                        if (total_usage > reserved_peer_usage * num_peers) oversized = true;
                    }
                    if (!oversized) break;
                    // Find worst peer.
    ...
    // This is not generally true, but must be true for this fuzz target.
    assert(real->TotalLatencyScore() == real->CountAnnouncements());
    ...
    for (unsigned tx = 0; tx < NUM_TX; ++tx) {
        bool sim_have_tx = have_tx_fn(tx);
        if (sim_have_tx) orphan_usage += GetTransactionWeight(*txn[tx]);
